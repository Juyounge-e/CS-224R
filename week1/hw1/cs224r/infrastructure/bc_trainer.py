...
class BCTrainer:
    ...
    def collect_training_trajectories(
        ...
        # CHECKLIST:
        # [ ] BC는 전문가 데이터 로드, dagger는 현재 정책 trajs 수집(itr == 0일 때는 전문가 데이터 로드)
        # [ ]  envsteps_this_batch trajs를 여려개 합한 총 envsteps 수 
        ...
        if itr == 0 and load_initial_expertdata: # 전문가 데이터 단순 로드
            with open(load_initial_expertdata, 'rb') as f:
                paths = pickle.load(f)
            envsteps_this_batch = sum([utils.get_pathlength(path) for path in paths]) # 로드한 전문가 trajs의 timsestep 합산
            print(f"전문가 데이터에서 trajectory {len(paths)}개, 총 {envsteps_this_batch} step을 불러왔습니다.")
        ...
        else:  # sample_trajectories를 이용하여 현재 agent 데이터 수집
            paths, envsteps_this_batch = utils.sample_trajectories(
                self.env, collect_policy, self.params['batch_size'], self.params['ep_len']
                )
            print(f"현재 policy로 trajectory {len(paths)}개, 총 {envsteps_this_batch} step을 수집했습니다.")
        ...
        if self.log_video:            
            # FIXME look in utils and implement sample_n_trajectories
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        # CHECKLIST:
        # [ ] dagger에서 수집한 trajs의 action을 전문가의 action으로 relabeling

...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        # CHECKLIST:
        # [ ] action을 반환하는 함수 구현. 이 함수는 policy의 forward 함수를 호출해야 함.
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        """
        # CHECKLIST:
        # [ ] forward pass를 구현하는 함수 구현, action을 리턴함 
    ...
    def update(self, observations, actions):
        ...
        """
        # CHECKLIST:
        # [ ] policy를 업데이트 하는 함수, loss계산, gradient로 파라미터 업데이트 
        ...
        action_predicted = self.forward(observations)
        loss = F.mse_loss(action_predicted, actions)
